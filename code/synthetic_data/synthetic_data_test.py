...
Script to test lead-lag detection and clustering pipeline on synthetic data
...
T_range = [250]  # 2p, 5p
p_range = [100]  # 20
num_clusters_range = [5, 10]  # 2, 5, 10
...
correlation_method_range = ['pearson', 'kendall', 'distance', 'mutual_information']  # 'spearman'

method_param_grid_dict = [
    dict(method=['signature']),
    dict(method=['ccf_at_lag'], lag=[1], correlation_method=correlation_method_range),
...
cluster_param_grid_dict = dict(
    cluster_method=['naive', 'degree_disc_bib', 'hermitian', 'hermitian_rw', 'co_clustering_left', 'co_clustering_right']
)
