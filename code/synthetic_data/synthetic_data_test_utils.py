...
def get_cluster_labels(lead_lag_matrix, num_clusters, cluster_method):
    if cluster_method == 'hermitian':
        cluster_labels = herm_matrix_algo.get_clustering(lead_lag_matrix, num_clusters, None)
    elif cluster_method == 'hermitian_rw':
        cluster_labels = herm_matrix_algo.get_clustering(lead_lag_matrix, num_clusters, 'rw')
    elif cluster_method == 'co_clustering_left':
        cluster_labels = co_clustering.get_clustering(lead_lag_matrix, num_clusters, cluster_edge_direction='left')
    elif cluster_method == 'co_clustering_right':
        cluster_labels = co_clustering.get_clustering(lead_lag_matrix, num_clusters, cluster_edge_direction='right')
    elif cluster_method == 'naive':
        cluster_labels = symmetrisation.naive(lead_lag_matrix, num_clusters)
    elif cluster_method == 'degree_disc_bib':
        cluster_labels = symmetrisation.degree_disc_bib(lead_lag_matrix, num_clusters)
    else:
...
def tabulate_to_results_frame(results_list, experiment_grid):
    ...
    # # re-ordering levels
    # level_order = ['T', 'p', 'num_clusters', 'sigma_z', 'sigma_eps', 'dependence', 'method', 'lag', 'max_lag', 'correlation_method',
    #                'cluster_method']
    # level_order = [level for level in level_order if(level in index.names)]
    # # level_order
    # results_frame = results_frame.reorder_levels(level_order)
    # # sorting dataframe
    # results_frame = results_frame.sort_values(level_order)
        ...
        for data_param in data_param_grid:
            ...
            for method_param in method_param_grid:
                ...
                for cluster_param in cluster_param_grid:
                    ...
                    # assume clustering algorithm knows true number of clusters
                    if data_param['dependence'] == 'heterogeneous':
                        num_clusters = data_param['n_factors'] * data_param['num_clusters']
                    else:
                        num_clusters = data_param['num_clusters']

                    try:
                        cluster_labels = get_cluster_labels(lead_lag_matrix, num_clusters, **cluster_param)
